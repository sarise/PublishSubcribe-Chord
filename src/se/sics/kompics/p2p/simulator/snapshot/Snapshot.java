...
public static void setFingers(PeerAddress address, PeerAddress[] fingers) {
	...
	private static String reportDetailes() {
		...
		str += "finger: " + verifyFingers(peersList) + "\n";
...
private static String verifyFingers(PeerAddress[] peersList) {
	...
	private static String verifyLonglinks(PeerAddress[] peersList) {
		...
		for (int i = 0; i < peersList.length; i++) {
			...
			count += (Peer.FINGER_SIZE - peer.getLonglinksSize());
	...
	private static String verifyFriendlinks(PeerAddress[] peersList) {
		...
		for (int i = 0; i < peersList.length; i++) {
			...
			count += (Peer.FINGER_SIZE - peer.getFriendlinksSize());
...
//-------------------------------------------------------------------
	...
	private static void generateGraphVizReport() {
		...
		for (PeerAddress peer : peers.keySet()) {
			...
			PeerAddress[] fingers = new PeerAddress[info.getLonglinksSize()];
			...
			fingers = info.getFingers();
			
			for (int j = 0; j < fingers.length; j++) {
				if (fingers[j] != null ){
					neighborLabel = fingers[j].getPeerId().toString();
					str += fingers[j].getPeerId() + " [ style = filled, label = \"" + neighborLabel + "\" ];\n";
					str += peer.getPeerId() + "--" + fingers[j].getPeerId() + "[ color = " + color[i] + " ];\n";
					}
			}
